...
const MensFashion = () => {
  ...
  const handleAddToCart = (product) => {
    ...
    // Using react-hot-toast for success notification
    ...
    setQuantity(prev => ({ ...prev, [product.id]: 1 })); // Reset quantity to 1 after adding
  };
  ...
  const handleCategoryChange = (category) => {
    ...
    toast(`Filtered by: ${category === 'All' ? 'All Categories' : category}`, {
      duration: 1500,
      position: 'bottom-center',
      icon: '👔', // Custom icon for category filter
    });
  };

  const handleStyleChange = (style) => {
    setStyleFilter(style);
    // Notify when style changes
    toast(`Filtered by style: ${style === 'All' ? 'All Styles' : style}`, {
      duration: 1500,
      position: 'bottom-center',
      icon: '👖', // Custom icon for style filter
    });
  };
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {styles.map(style => (
              <button
                key={style}
                onClick={() => handleStyleChange(style)} // Changed to handleStyleChange
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {categories.map(category => (
              <button
                key={category}
                onClick={() => handleCategoryChange(category)} // Changed to handleCategoryChange
                className={`px-3 py-1 rounded-full text-sm ${
