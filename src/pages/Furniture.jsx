...
const Furniture = () => {
  ...
  const handleCategoryFilter = (category) => {
    ...
    toast(`${category} furniture displayed.`, {
      icon: '🛋️', // Custom icon for category filter
      duration: 1500,
    ...
    <div className="max-w-6xl mx-auto p-6 font-poppins mt-20">
      ...
      >
        <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col items-center justify-center text-white text-center">
          ...
          <button
            className="mt-5 bg-yellow-500 text-black px-6 py-2 rounded font-medium hover:bg-yellow-400 transition"
            onClick={() => toast('Exploring our exquisite collection!', { icon: '✨' })}
          >
            Shop Now
          </button>
        </div>
      ...
      <div className="flex justify-center gap-4 mt-10">
        {categories.map((category, index) => (
          <button
            key={index}
            onClick={() => handleCategoryFilter(category)} // Use handleCategoryFilter
        ...
        {filteredFurniture.map((product) => (
          ...
          >
            ...
            <div className="p-4 text-center">
              ...
              <button
                className="mt-4 bg-teal-600 text-white px-6 py-2 rounded-full font-medium hover:bg-teal-800 transition"
                onClick={() => handleBuyNowClick(product.name)} // Attach Buy Now handler
              >
                Buy Now
              </button>
            </div>
